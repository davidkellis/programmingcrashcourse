...
\`\`\`javascript
const temperature_outside = Math.floor(Math.random() * 71) + 30; // 30 to 100
...
The way we can make decisions in our JavaScript programs is to use \`nr: if\` / \`nr: else if\` / \`nr: else\` expressions.
...
\`\`\`javascript
const temperatureInTheHouse = 70
...
### Special rules in JavaScript

JavaScript has specific rules about how we must write \`nr: if\`, \`nr: else if\`, and \`nr: else\` expressions (conditional expressions):
...
function playHubcapOfHavoc() {
  const spinResult = spinTheWheel()
  if (spinResult === 0) {
...
JavaScript has a compact form of if/else expression for this case: \`nr: condition ? A : B\`. It evaluates to \`nr: A\` when the condition is \`nr: true\`, and evaluates to \`nr: B\` when the condition is \`nr: false\`.
...
For example, in JavaScript, we could count down using a while loop:
...
There are also other ways to do the same thing. The next section will explain the ways we can write a loop in JavaScript.
...
const names = ["Alice", "Bill", "Bob", "Charlie"];
const namesThatStartWithUpperCaseB = [];
for (const name of names) {
...
const names = ["Alice", "Bill", "Bob", "Charlie"];
const namesThatStartWithUpperCaseB = names.filter(name => name.startsWith("B"));
namesThatStartWithUpperCaseB;
\`\`\`
...
const names = ["Alice", "Bob", "Charlie"];
const namesThatDoNotStartWithUpperCaseB = names.filter(name => !name.startsWith("B"));
namesThatDoNotStartWithUpperCaseB;
\`\`\`

### Searching for an item in a collection

We can use a loop to search for an item in a collection, but there are built-in methods that make it easy in JavaScript.
...
const numbers = [5, 80, 138, 1, 36, 101];
let i = 0;
let firstNumberGreaterThan100 = null;
...
const numbers = [5, 80, 138, 1, 36, 101];
const firstNumberGreaterThan100 = numbers.find(number => number > 100);
firstNumberGreaterThan100;
\`\`\`
