...
class LocalContentService {
   ...
   */
  async getAllSections(_language: string): Promise<TutorialSection[]> {
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 100))

    // For now, return the same content regardless of language
    // In the future, we could have language-specific content
   ...
   */
  async getSection(sectionId: string, _language: string): Promise<TutorialSection | null> {
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 100))
  ...
  /**
   * Get code snippets for a section
   */
  ...
  /**
   * Add a new section (for future content management)
   */
  addSection(section: TutorialSection): void {
    TUTORIAL_SECTIONS.push(section)
    // Sort by order
    TUTORIAL_SECTIONS.sort((a, b) => a.order - b.order)
  }
  ...
  updateSection(sectionId: string, updates: Partial<TutorialSection>): boolean {
    const index = TUTORIAL_SECTIONS.findIndex(s => s.id === sectionId)
    if (index === -1) return false

    TUTORIAL_SECTIONS[index] = { ...TUTORIAL_SECTIONS[index], ...updates }
    return true
  }
