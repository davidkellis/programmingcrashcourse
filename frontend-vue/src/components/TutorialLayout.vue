                ...
                <h1 class="tutorial-title">
                  <RouterLink
                    ...
                    :to="{
                      ...
                      params: {
                        language:
                          (route.params.language as string) ||
                          uiState.selectedLanguage?.id ||
                          DEFAULT_LANGUAGE.id,
              ...
              <div class="header-center">
                <div class="nav-group">
                  <button
                    ...
                    :aria-label="
                      previousSection ? `Go to ${previousSection.title}` : 'No previous section'
                    "
                  >
                    ← Previous
                  </button>
...
const replState = ref<REPLState & { lastExecutedCode?: string }>({
  sessionId: null,
  isExecuting: false,
  history: [],
  currentInput: '',
...
const gridStyle = computed(() => {
  ...
  if (replLayout.value.position === 'right') {
    ...
    document.documentElement.style.setProperty('--repl-width', `${replW}px`)
    return {
      gridTemplateColumns: `minmax(320px, 1fr) minmax(0, ${replW}px)`,
...
const handleREPLPositionChange = (
...
const navigateToSection = (sectionId: string) => {
  ...
  const lang =
    (route.params.language as string) || uiState.value.selectedLanguage?.id || DEFAULT_LANGUAGE.id
  router.push({ name: 'section', params: { language: lang, sectionId } })
}
const navigateToPrevious = () => {
  if (previousSection.value) navigateToSection(previousSection.value.id)
}
const navigateToNext = () => {
  if (nextSection.value) navigateToSection(nextSection.value.id)
}

const handleClickOutside = (event: Event) => {
  const target = event.target as HTMLElement
  if (!target.closest('.toc-dropdown')) isTOCVisible.value = false
}
...
watch(
  () => uiState.value.selectedLanguage,
  (newLanguage) => {
    if (newLanguage && !replState.value.sessionId) createREPLSession(newLanguage)
  },
)
...
.dock-right .repl-pane {
  grid-column: 2;
  grid-row: 1;
}

.dock-bottom .content-pane {
  grid-column: 1;
  grid-row: 1;
  overflow: auto;
  min-height: 0;
